...
resource "google_compute_router" "mk_news_router" {
  ...
  depends_on = [google_compute_network.mk_news_vpc]
}

# ========================================
# 고정 IP 주소 (NAT Gateway용)
# ========================================
...
resource "google_compute_router_nat" "mk_news_nat" {
  ...
  depends_on = [
    google_compute_router.mk_news_router,
    google_compute_address.mk_news_static_ip
  ]
}
...
# IAM 정책은 Cloud Run 서비스 생성 후 수동으로 설정
...
#   member   = "allUsers"
#   
#   depends_on = [google_cloud_run_v2_service.mk_news_admin]
# }
